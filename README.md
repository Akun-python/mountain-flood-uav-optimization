...
<p align="center">
  📦 <b>80 货箱</b> &nbsp;·&nbsp; 🛸 <b>8 运输机 + 2 中继</b> &nbsp;·&nbsp; ⚡ <b>26 架次 / 约 115 min / 零迟到</b> &nbsp;·&nbsp; 📡 <b>盲区为零</b>
</p>
...
> 以下摘要与论文 `main.tex` 摘要一致，采用 **2026-09-25 进化 v24（末班减载，联合完工 6921 s）** 核验的最新口径（问题二 26 架次 / 115 min / 70.86 kWh，问题三运输 6897 s、联合完工 6921 s、中继合计 3.53 kWh、盲区为零）。
...
**问题二：异构无人机多点多架次调度。** 以架次为决策单元，按时限贪心组批生成初始解并拆出紧急架次，再并行比较模拟退火、遗传算法、自适应大邻域搜索、禁忌搜索与 GRASP，并以多权重-多种子强化搜索收敛 Pareto 前沿。调度层按截止期优先，并耦合无人机与电池双资源就绪队列求可行派单。选取零迟到的禁忌搜索均衡方案：26 架次，其中 A 型 12、B 型 7、C 型 7，完工时间约 115 min，总能耗 70.86 kWh，全部 80 箱按各自时限交付，加权迟到为零。相较原遗传算法均衡方案，完工提前约 24 min（17.3%）、能耗降低约 6.5 kWh；若以能耗为先，可选用 21 架次方案（约 131 min、62.46 kWh）。

**问题三：通信约束下运输与中继联合调度。** 建立频率 2400 MHz 的自由空间传播加地形遮挡的电波模型，接收门限为 -90 dBm，直连、接入、回程链路上限为 122、116、126 dB，并按 30 m 步长沿航线逐点判定遮挡。覆盖采样确定三个中继布设点，W 点服务西与北片区，E 点服务东片区，N 点服务 S004；运输任务分散后，初始布设即实现全覆盖。以问题二的 26 架次方案为运输层输入，两架中继按时间分片复用：W 点服务时段 778 至 6502 s，能耗 1.979 kWh；E 点服务时段 800 至 3774 s，能耗 1.102 kWh；N 点服务时段 5594 至 6212 s，能耗 0.450 kWh，中继合计约 3.53 kWh，回程链路全部可行。协同方案保持 26 架次、运输完工约 115 min、运输能耗 70.86 kWh，联合完工 6921 s（约 115.4 min）由中继 1 返场决定，较原协同基线 6960 s 提前约 39 s；全部 80 箱按时交付、加权迟到为零，覆盖盲区为零。

**问题四：任务分区与资源配置。** 按中继覆盖结构给出三组与两组两种分区，对每组以最小机队搜索确定机型与电池数量，再叠加冗余备份核算库存。结果表明，若按组完全独立配置，两种分区均超出库存，例如两组方案需 A 型机 6 架超出 2 架、B 型机 5 架超出 3 架、A 型电池 8 组超出 2 组、中继 6 架超出 4 架；分三组时重复配置更多。给定库存下更优组织为任务分区、资源池化，即分区仅用于任务组织，机队与中继仍集中调度并时间分片复用，问题三的协同方案以 8 架运输机、14 组电池、2 架中继恰好满足库存，实现零迟到与零盲区；若坚持独立配置，需增配 A 型机 2 至 4 架、B 型机 3 至 5 架、A 型电池 2 至 4 组与中继 4 至 7 架。
...
|---|---:|---:|---:|
| 架次数 | 18 | 26 | 26 + 中继 3 班 |
| 完工时间 | 累计约 9.1 h | 约 115 min（6897 s） | 联合 6921 s（115.4 min） |
| 总能耗 | 59.02 kWh | 70.86 kWh | 约 74.39 kWh（含中继 3.53 kWh） |
...
# 2. 强化搜索并导出冠军（26 架次 / 约 115 min / 70.86 kWh）
python -X utf8 求解代码与结果/实验/champion_search.py --export

# 3. 运输-中继联合调度（中继三班 3.53 kWh，联合完工 6921 s）
...
- **v24（问题二/三 · S004-FOD 末班转出，26 架完工纪录刷新）**：把 C 型末班 f05 上的
  S004-FOD-01（8 kg）转移到 f20（U08，S003 段，S004-HYG 已在该架次顺路投递），
  不动 f11/f4/f22，避开 v19/v20 的失败模式（EDF 排序扰动与 B 链瓶颈）。结果：
  **运输完工 6959.81 → 6896.52 s（-63.3 s，-0.91%），打破 v19 认定的 26 架完工纪录
  （v19 须 27 架才达 6896.51 s）；联合完工 6959.8 → 6921 s（-38.8 s，中继 1 返场
  成为联合口径上界）；运输能耗 70.705 → 70.861 kWh（+0.156）；中继 3.531 kWh、
  偏移 {f4:3300, f22:1000} 与三班时段均不变；26 架、零迟到、cover_bad=0、mpen=0。**
  同时修正 `p3_margins.py` 布设点坐标口径（内部 E/N 仍为旧值，改为 v17 采纳位置后
  与论文余量表完全一致，W 因 +4 采样点 982→986）。负结果归档：f11 加箱连锁
  （f09 挤 U01 尾，W 尾段 6742）、f4 加箱超 A 型能量上限、27 架新架次排位靠后、
  f09 换 B 型推晚 f22——均严格端到端拒绝。见 `结果/进化_v24/`。
...
|---|---:|---:|---:|---:|
| **完工冠军（Tabu，本文选取）** | **26** | **6897** | **70.86** | **0** |
...
> 1.00 / 1.11 / 9.40 dB，均无低于 1 dB 的采样点（进化 v24 复核不变）。

这三个点也自然形成了问题四的任务分区依据。

### 7.3 覆盖审核与中继时序规划

对问题二的 26 架次禁忌搜索方案按 30 m 步长做逐点通信审核，共得到 1390 个需中继采样点。初始中继布设即实现全覆盖：西点、东点与北点三个布设点的接入与回程链路在全部采样点可行，无需任何人工调整。中继单架次能源预算为 2.56 kWh，三班均未超预算。
...
|---|---:|---:|---:|
| 运输架次 | 26 | 26 | 不变 |
| 完工时间 | 6897 s | 6921 s（中继1返场） | +24 s（0.36%） |
| 运输总能耗 | 70.86 kWh | 70.86 kWh | 不变 |
...
- 运输架次保持 26 架次，运输完工约 115 min（6897 s，较问题二冠军提前 63 s），联合完工 6921 s（中继 1 返场决定，较原协同基线 6960 s 提前约 39 s），运输能耗 70.86 kWh；
- 中继三班共 3.53 kWh，均满足单班 2.56 kWh 能源预算；
- 1390 个需中继采样点全部覆盖，盲区为 0；
- 全部飞行阶段通信盲区为 0；
- 两架中继的转场衔接无冲突。
...
生成运输初始解，在相同调度解码器与墙钟预算下并行对比模拟退火、遗传算法、自适应大邻域搜索、禁忌搜索与 GRASP 五类元启发式，并执行多权重-多种子强化搜索收敛 Pareto 前沿；禁忌搜索在零迟到硬约束下取得完工冠军（26 架次 / 115 min / 70.86 kWh），节能备选 21 架次 / 131 min / 62.46 kWh；使用资源解码器安排无人机、电池和起降时间，输出 `p2_results.json` 与 Pareto 对照结果。

### 第五步：求解问题三

对 Tabu 冠军方案按 30 m 步长做逐点通信审核，识别需中继采样点（共 1390 个）；搜索并确定 W/E/N 三个中继布设点，安排两架中继、三个服务时段；初始布设即实现全覆盖，形成运输—中继协同方案。
...
> 注：论文最终采用 Tabu 完工冠军口径（26 架次 / 115 min / 70.86 kWh + 初始布设即全覆盖的中继方案，联合完工 6921 s）。历史版本归档见 `求解代码与结果/结果/进化_v3` 至 `进化_v10`；若需按论文原遗传算法方案（24 架次 / 8342.1 s / 77.31 kWh）复现，可运行 `求解代码与结果/方案脚本/v9w_q3_final.py`。具体版本与结果文件说明见 [`求解代码与结果/README.md`](求解代码与结果/README.md)。
...
| 问题二 | 通过截止期优先、五族元启发式对比与多权重-多种子强化搜索（Tabu 完工冠军）得到 26 架次方案，完工约 6897 s，总能耗 70.86 kWh，零迟到；较 GA 方案完工提前约 17.3%、能耗降低约 6.5 kWh。 |
| 问题三 | 通过 DEM 遮挡判定与自由空间链路预算确定 W/E/N 三个中继点，两架中继分三个时段服务（W 1.979 / E 1.102 / N 0.450 kWh，合计约 3.53 kWh；东、北点经余量 ≥1 dB 约束下的位置微调），初始布设即全覆盖 1390 个需中继采样点，26 架次运输全程通信覆盖，盲区为零；运输完工 6897 s，联合完工 6921 s（中继 1 返场决定，较原协同基线提前约 39 s），中继约束波纹很小（+0.36%）。 |
...
3. 运输与中继的联合调度并非零和博弈：以已核验的运输方案为输入、在几何与能源层面补齐覆盖，即可实现零盲区且运输能耗不变（完工代价仅 0.36%），运输层结构调整还可带来额外完工收益。
