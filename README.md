...
> 以下摘要与论文 `main.tex` 摘要一致，采用 **2026-09-25 进化 v17（余量约束位置优化）** 核验的最新口径（问题二 26 架次 / 116 min / 70.71 kWh，问题三端到端约 121 min、中继合计 3.42 kWh、盲区为零）。
...
**问题三：通信约束下运输与中继联合调度。** 建立频率 2400 MHz 的自由空间传播加地形遮挡的电波模型，接收门限为 -90 dBm，直连、接入、回程链路上限为 122、116、126 dB，并按 30 m 步长沿航线逐点判定遮挡。覆盖采样确定三个中继布设点，W 点服务西与北片区，E 点服务东片区，N 点服务 S004；运输任务分散后，初始布设即实现全覆盖。以问题二的 26 架次方案为运输层输入，两架中继按时间分片复用：W 点服务时段 1078 至 6802 s，能耗 1.979 kWh；E 点服务时段 1406 至 4074 s，能耗 1.009 kWh；N 点服务时段 5894 至 6446 s，能耗 0.429 kWh，中继合计约 3.42 kWh，回程链路全部可行。协同方案保持 26 架次、完工时间约 121 min、运输能耗 70.71 kWh，全部 80 箱按时交付、加权迟到为零，覆盖盲区为零。
...
| 总能耗 | 59.02 kWh | 70.71 kWh | 约 74.13 kWh（含中继 3.42 kWh） |
...
# 3. 运输-中继联合调度（中继三班 3.42 kWh，端到端约 121 min）
...
| R2-01 | 东点 E | [1406, 4074] s | 1.009 kWh | — |
| R2-02 | 北点 N | [5894, 6446] s | 0.429 kWh | — |
...
| 中继总能耗 | 未计 | 3.42 kWh | 三班保障 |
| 通信盲区 | 未验证 | 0 | 初始布设即全覆盖 |

最终方案实现：

- 80 箱货物全部按时送达（三档时限零违反、加权迟到为零）；
- 运输架次保持 26 架次，完工时间约 121 min，运输能耗 70.71 kWh；
- 中继三班共 3.42 kWh，均满足单班 2.56 kWh 能源预算；
...
| 问题三 | 通过 DEM 遮挡判定与自由空间链路预算确定 W/E/N 三个中继点，两架中继分三个时段服务（W 1.979 / E 1.009 / N 0.429 kWh，合计 3.42 kWh；东、北点经余量 ≥1 dB 约束下的位置微调），初始布设即全覆盖 1386 个需中继采样点，26 架次运输全程通信覆盖，盲区为零。 |
