...
**问题二：异构无人机多点多架次调度。** 以架次为决策单元，按时限贪心组批生成初始解并拆出紧急架次，再并行比较模拟退火、遗传算法、自适应大邻域搜索、禁忌搜索与 GRASP。调度层按截止期优先，并耦合无人机与电池双资源就绪队列求可行派单。选取零迟到的禁忌搜索均衡方案：21 架次，其中 A 型 7、B 型 6、C 型 8，完工时间约 8337 s，总能耗 68.06 kWh，全部 80 箱按各自时限交付，加权迟到为零。相较原遗传算法均衡方案，架次减少 3 架、完工提前约 5 s、能耗降低约 9.25 kWh。

**问题三：通信约束下运输与中继联合调度。** 建立频率 2400 MHz 的自由空间传播加地形遮挡的电波模型，接收门限为 -90 dBm，直连、接入、回程链路上限为 122、116、126 dB，并按 30 m 步长沿航线逐点判定遮挡。覆盖采样确定三个中继布设点，W 点服务西与北片区，E 点服务东片区，N 点服务 S004。以问题二的 21 架次方案为运输层输入，两架中继按时间分片复用：W 点服务时段 853 至 7936 s，能耗 2.394 kWh；E 点服务时段 760 至 3485 s，能耗 1.116 kWh；N 点服务时段 1913 至 4858 s，能耗 1.252 kWh。自动化覆盖诊断发现东点存在 4 个未覆盖采样点，将东点海拔上调 300 m 后全部消除，回程链路仍可行。协同方案保持 21 架次、完工时间 8337 s、运输能耗 68.06 kWh，全部 80 箱按时交付、加权迟到为零，覆盖盲区为零。
...
**灵敏度分析**表明模型结论稳健：安全余量扫描复现安全效率权衡并呈现机型结构切换；多目标权重在合理范围内只改变拆分粒度，不改变 C 型瓶颈结构；中继 W 点最长连续服务 7083 s，占单架次能源上限对应时长约 8378 s 的 84.5%，悬停功率扰动正负 10% 后方案不变；通信余量统计中 W 点最紧 0.5 dB、E 点次紧 1.0 dB、N 点最紧 13.4 dB，余量下压 1 dB 时仅 W 点 4 个采样点低于门限，小幅移位 W 点布设位置即可恢复全部覆盖。
...
对照方案（同一调度解码器与墙钟预算下的 Tabu 权重扫描）：

| 方案 | 架次数 | 完工时间(s) | 能耗(kWh) | 加权迟到 |
|---|---:|---:|---:|---:|
| **均衡（Tabu，本文选取）** | **21** | **8337** | **68.06** | **0** |
...
相较原遗传算法均衡方案（24 架次 / 8342 s / 77.31 kWh），Tabu 冠军方案架次减少 3 架、完工提前约 5 s、能耗降低约 9.25 kWh（约 12%）。问题二的方案为问题三提供了运输基线，但此时尚未加入全程通信约束。
...
对问题二的 21 架次禁忌搜索方案按 30 m 步长做逐点通信审核，共得到 1259 个需中继采样点。初始布设下西点与北点均已覆盖，仅东点存在 4 个未覆盖采样点；坐标搜索表明，将东点海拔由 542.3 m 上调至 842.3 m 即可使全部采样点被覆盖，且中继至地面站的回程链路仍可行。中继单架次能源预算为 2.56 kWh，三班均未超预算。

将所有需要中继的飞行阶段按布设点归类，再把相邻或时间重叠的任务合并为服务窗口。最终采用两架中继、三个服务时段：

| 中继任务 | 位置 | 服务时间 | 能耗 | 返航荷电状态 |
|---|---:|---:|---:|---:|
| R1-01 | 西点 W | [853, 7936] s | 2.394 kWh | 约 25.2% |
| R2-01 | 东点 E | [760, 3485] s | 1.116 kWh | — |
| R2-02 | 北点 N | [1913, 4858] s | 1.252 kWh | — |
...
|---|---:|---:|---:|
| 运输架次 | 21 | 21 | 不变 |
| 完工时间 | 8337.0 s | 8337.0 s | 不变 |
| 运输总能耗 | 68.06 kWh | 68.06 kWh | 不变 |
| 中继总能耗 | 未计 | 4.76 kWh | 三班保障 |
| 通信盲区 | 未验证 | 0 | 东点海拔上调 300 m 后清零 |

最终方案实现：

- 80 箱货物全部按时送达（三档时限零违反、加权迟到为零）；
- 运输架次保持 21 架次，完工时间 8337.0 s，运输能耗 68.06 kWh；
- 中继三班共 4.76 kWh，均满足单班 2.56 kWh 能源预算；
- 1259 个需中继采样点全部覆盖，东点 4 个未覆盖采样点经海拔上调后清零；
- 全部飞行阶段通信盲区为 0；
- 两架中继的转场衔接无冲突。

该结果说明通信约束并不一定会增加运输成本：联合调度直接以 Tabu 冠军方案为输入，覆盖缺口通过中继坐标微调在几何层面消除，运输效率零损失，运输与中继的联合调度并非零和博弈。
...
生成运输初始解，在相同调度解码器与墙钟预算下并行对比模拟退火、遗传算法、自适应大邻域搜索、禁忌搜索与 GRASP 五类元启发式，禁忌搜索在零迟到硬约束下取得最优权衡（21 架次 / 8337 s / 68.06 kWh）；使用资源解码器安排无人机、电池和起降时间，输出 `p2_results.json` 与 Pareto 对照结果。

### 第五步：求解问题三

对 Tabu 冠军方案按 30 m 步长做逐点通信审核，识别需中继采样点（共 1259 个）；搜索并确定 W/E/N 三个中继布设点，安排两架中继、三个服务时段；对东点布设做海拔上调 300 m 的坐标修复，消除全部未覆盖采样点，形成运输—中继协同方案。
...
# 2. 导出 Tabu 冠军配置到 p2_results.json
...
> 注：论文最终采用 Tabu 冠军口径（21 架次 / 8337 s / 68.06 kWh + 东点海拔上调 300 m 的中继覆盖修复）。若按旧版遗传算法方案（24 架次 / 8342.1 s / 77.31 kWh）复现，可运行 `求解代码与结果/方案脚本/v9w_q3_final.py`。具体版本与结果文件说明见 [`求解代码与结果/README.md`](求解代码与结果/README.md)。
...
| 问题二 | 通过截止期优先、五族元启发式对比（Tabu 冠军）与无人机—电池联合解码得到 21 架次方案，完工约 8337.0 s，总能耗 68.06 kWh，零迟到；较 GA 方案能耗降低约 12%。 |
| 问题三 | 通过 DEM 遮挡判定与自由空间链路预算确定 W/E/N 三个中继点，两架中继分三个时段服务（W 2.394 / E 1.116 / N 1.252 kWh），东点海拔上调 300 m 修复 4 个未覆盖采样点，21 架次运输全程通信覆盖，盲区为零。 |
