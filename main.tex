...
优先，并耦合无人机与电池双资源就绪队列求可行派单。选取零迟到的禁忌搜索均衡方案：
26 架次，其中 A 型 12、B 型 7、C 型 7，完工时间约 6960\,s，总能耗 70.71\,kWh，
全部 80 箱按各自时限交付，加权迟到为零。相较原遗传算法均衡方案，完工提前约
1382\,s（16.6\%）、能耗降低约 6.6\,kWh；若以能耗为先，可选用 21 架次方案
（约 7837\,s、62.46\,kWh）。
...
布设点，W 点服务西与北片区，E 点服务东片区，N 点服务 S004；运输任务分散后，
初始布设即实现全覆盖。以问题二的 26 架次方案为运输层输入，两架中继按时间分片
复用：W 点服务时段 1078 至 6802\,s，能耗 1.979\,kWh；E 点服务时段 1406 至
4074\,s，能耗 1.046\,kWh；N 点服务时段 5894 至 6446\,s，能耗 0.521\,kWh，
中继合计约 3.55\,kWh，回程链路全部可行。协同方案保持 26 架次、完工时间约
7260\,s、运输能耗 70.71\,kWh，全部 80 箱按时交付、加权迟到为零，覆盖盲区为零。

\textbf{问题四：任务分区与资源配置。}按中继覆盖结构给出三组与两组两种分区，
对每组以最小机队搜索确定机型与电池数量，再叠加冗余备份核算库存。结果表明，
若按组完全独立配置，两种分区均超出库存，例如两组方案需 A 型机 6 架超出 2 架、
B 型机 5 架超出 3 架、中继 6 架超出 4 架；分三组时重复配置更多。给定库存下
更优组织为任务分区、资源池化，即分区仅用于任务组织，机队与中继仍集中调度并
时间分片复用，问题三的协同方案以 8 架运输机、14 组电池、2 架中继恰好满足库存，
实现零迟到与零盲区；若坚持独立配置，需增配 A 型机 2 至 4 架、B 型机 3 至 5 架、
A 型电池 2 至 4 组与中继 4 至 7 架。

\textbf{灵敏度分析}表明模型结论稳健：安全余量扫描复现安全效率权衡并呈现机型
结构切换；多目标权重在合理范围内只改变拆分粒度，不改变 C 型瓶颈结构；中继
W 点最长连续服务 5724\,s，占单架次能源上限对应时长约 8378\,s 的 68.3\%，
悬停功率扰动正负百分之十后方案不变；通信余量统计中 W 点最紧 1.00\,dB、E 点
1.03\,dB、N 点 13.43\,dB，三个布设点均无低于 1\,dB 的采样点，余量下压 1\,dB
