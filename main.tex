...
自学习穿越完工非单调走廊）与宽带束搜索（接受完工暂升—池负载均衡回升的中间态）
逐级精炼，收敛 Pareto 前沿。调度层按截止期优先，并耦合无人机与电池双资源就绪
队列求可行派单；评估按严格口径执行，\textbf{每个货箱的投递时刻所属服务区必须与
其箱号归属区一致}（区一致零违例），凡"捎带"结算者一律判不可行。选取零迟到的
精炼冠军方案：27 架次，其中 A 型 13、B 型 8、C 型 6，完工时间约
109.5\,min（6571.4\,s），总能耗 70.68\,kWh，全部 80 箱按各自时限、投送至各自
指定服务区交付，加权迟到为零。相较传统元启发式最优方案（25 架次、118.5\,min、
70.28\,kWh）完工提前约 9.0\,min（7.6\%），结构上三池负载（5755/5810/5990\,s）
均衡化、消除了单池瓶颈；若以能耗为先，可选用 21 架次方案
（约 130.6\,min、64.48\,kWh）或 19 架次方案（约 132\,min、60.40\,kWh）。
