...
如表 \ref{tab:pareto} 所示。本文选取完工口径冠军：26 架次、完工约 115\,min、
能耗 70.86\,kWh，较原遗传算法均衡方案（24 架次、139\,min、77.31\,kWh）完工提前
约 24\,min（17.3\%）、能耗降低约 6.5\,kWh；若以能耗为先，21 架次方案约
...
\midrule
完工冠军（Tabu，本文选取） & \textbf{26} & \textbf{6897} & \textbf{70.86} & \textbf{0} \\
...
约束满足且加权迟到为零，完工时间约 115\,min，总能耗 70.86\,kWh。图
...
\caption{26 架次能耗构成（按机型着色，总量 70.86\,kWh）}
\label{fig:energydecomp}
\end{figure}

\begin{enumerate}
\item \textbf{C 型瓶颈}：7 个 C 架次承载了主要重载区任务，U07 与 U08 两架
C 型无人机几乎连续出勤至约 115\,min（末班 f05 返场 6894\,s），与 B 型链末班
f26（返场 6897\,s）共同构成完工时间瓶颈；A、B 型负责
