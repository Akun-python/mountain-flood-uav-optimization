...
载荷、能耗与返航荷电状态的可核验映射是两层任务的共同基础：载荷越大，等效
航程越短，单位里程能耗越高，架次能耗随之上升，因此最大安全载荷由质量上限、
体积上限与能量上限三者中的最紧者决定。

图 \ref{fig:demmap} 给出研究区地形与服务区分布：镇龙乡山地位于调度中心西南，
S002、S004、S008 等远区需跨越 400\,m 以上高差，是载荷与能耗约束最紧的区域；
三个中继布设点覆盖全部服务区通信需求（详见问题三）。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.96\textwidth]{figures/p1_dem_map.png}
\caption{研究区地形（30\,m DEM）、服务区、调度中心与中继布设点分布}
\label{fig:demmap}
\end{figure}
...
作业高度取服务区地面高程以上 30\,m，投递时延为基础交接时间（A/B 型 150\,s、
C 型 180\,s）加每箱交接时间（A/B 型 30\,s、C 型 36\,s），每次投送后从
30\,m 高度重新爬升。图 \ref{fig:altprof} 给出四条代表性
航线的 DEM 高程剖面：山脊横亘于调度中心与远区之间，爬升段在总飞行中占比大，
因此爬升能耗与时间必须显式建模，等效航程法把水平部分折算为能耗，爬升部分按
高度差单独计能。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.98\textwidth]{figures/p1_alt_profiles.png}
\caption{典型航线 DEM 高程剖面与巡航高度（虚线为巡航高度，点线为作业高度）}
\label{fig:altprof}
\end{figure}
...
受能量约束为 59.2 至 69.0\,kg。图 \ref{fig:qstar} 给出三机型各区域的载荷
对比，可见能量约束集中在 C 型远区。
...
按式 \eqref{eq:bin} 组批，全网共 18 架次、总能耗 59.02\,kWh、累计作业时间约
9.1\,h。表 \ref{tab:p1summary} 给出各服务区的组批概览：重载区由 C 型承担，
