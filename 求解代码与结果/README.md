...
├── 结果/          当前冠军结果 JSON 与进化归档（进化_v3 … 进化_v23）
...
- 问题三（p3_co2.json）：1386 个需中继采样点全部覆盖（西点 982、东点 294、北点 110），
  ...
  （1.979 kWh，返航荷电 38.2%）；中继 2 先东点 [1406, 4074] s（1.009 kWh，
  东点经余量 ≥1 dB 约束位置微调至 (109.2689, 23.0127, 650 m)）
  后北点 [5894, 6446] s（0.429 kWh，北点 (109.2343, 23.0592, 600 m)），
  中继合计 3.42 kWh，转场衔接无冲突；
  联合完工时间约 121 min，零迟到、零盲区；接入余量 W 1.00 / E 1.11 / N 9.40 dB
  均无低于 1 dB 采样点（p3_margins.json）。
- 独立校验：80 箱无重复无缺失；无不可行架次；硬时限零违反；通信盲区为零。
- 问题三/问题四持续优化实验（多框架对比、严格复核）见 `结果/进化_v17/README.md`；问题二尾部修复与联合完工最优性验证（v19）见 `结果/进化_v19/README.md`；问题二能耗边界与中继安全筛查（v20）见 `结果/进化_v20/README.md`；中继充电模型条件修正（v21）见 `结果/进化_v21/README.md`；W 窗口末端收口攻击（v22）见 `结果/进化_v22/README.md`；联合完工破局（v23，7259.81→6959.8 s）见 `结果/进化_v23/README.md`。
...
各进化版本（v3 … v18）的完整实验记录与结果归档见 `结果/进化_v*/README.md`。
