# -*- coding: utf-8 -*-
"""v79b：电池选择策略回放（修正 eval）——evaluate(data, fls, sch) + check_battery。"""
import sys, os, json
...
print('策略     完工s      能耗    硬  违规 临界 电池')
for pick in ['earliest', 'latest', 'socmax', 'socmin', 'roundrobin']:
    sch, _ = dispatch_pick(data, fls, pick)
    if sch is None:
        print('%-10s 调度失败' % pick); continue
    m = evaluate(data, fls, sch)
    vb = cb(data, sch, fls)
    print('%-10s %8.1f %8.2f %5s %4d %4d %4d' % (pick, m['makespan'], m['energy'], m['hard_ok'], m['violations'], m['critical'], len(vb)))
print('基准(默认): 6571.4 70.68')
