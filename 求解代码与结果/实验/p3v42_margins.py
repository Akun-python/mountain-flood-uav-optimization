# -*- coding: utf-8 -*-
"""v42 基线派单接入链路余量统计（互斥归属口径）。
输入：results/p2_results.json（29 架）+ p3v42_final.json（起点）。
输出：results/p3_margins.json。"""
import sys, os, json
...
for g in 'WEN':
    ...
    print('%s: n=%d min=%s avg=%s areas=%s' % (g, res[g]['n'], res[g]['min'],
                                               res[g]['avg'], res[g]['areas']))
