# -*- coding: utf-8 -*-
"""v42-τ：90min 构造 v10（全组合枚举装载 + 首飞批分布）：
C 6：S001×2、S002、S003、S004、S008+S007 跨区
B 6：S004 3+3、S005 3+3、S006 3+3（重区 3 箱 -01 组合）
A 12-13：S007 尾、S008 0、轻区 21、S001/S002/S003 尾
24-25 趟 80 箱；输出 min90_init.json。"""
...
def load(area, model, max_n, first_pass=False, exclude_first=False):
    """全组合枚举：n 从 max_n 降序，取质量最大可行组合（-01 优先/排除）。"""
    ...
    if first_pass:
        ...
        pool = [b for b in pool if not b.endswith('-01')]
    if not pool:
        return None, None
...
# C 6 趟：首飞批区前 3（S001/S002/S003），后 3 非首飞批
add('C', 'S001', 7, True)
add('C', 'S002', 7, True)
add('C', 'S003', 7, True)
add('C', 'S001', 7, False, True)
add('C', 'S004', 6, False, True)
# S008 5 + S007 跨区拼（C 第 6 趟）
if data.area_boxes['S008']:
    bs, f = load('S008', 'C', 6, True)
    if f:
        extra = []
        s7 = list(data.area_boxes['S007'])
        for b in s7:
            if len(f.route[0][1]) + len(extra) < 7:
                extra.append(b)
        f = Flight(f.fid, [(s, list(bb)) for s, bb in f.route] +
                   ([('S007', extra)] if extra else []), 'C', data)
        for b in extra:
            data.area_boxes['S007'].remove(b)
        f.fid = fid; fl.append(f); fid += 1
# B 6 趟：S004/S005/S006 各 2 趟 3 箱（首飞批区波 1-2）
for s in ('S004', 'S005', 'S006'):
    add('B', s, 3, True)
    add('B', s, 3, False)
# A 12 趟：轻区 7 区 2 箱 + 尾箱拼
light = ['S007', 'S008', 'S009', 'S010', 'S011', 'S012', 'S013', 'S014', 'S015']
for s in light:
    add('A', s, 2, True)
# 剩余拼（A 2 箱跨区 / 单区）
rem_box = []
