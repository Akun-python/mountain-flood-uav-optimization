...
## D. P3 合规重算实测补充（v28，`实验/p3v28_compliant.py`）

### D1【口径·已修复】P3 SA 目标权重失衡导致运输整体推迟
- **现象**：SA（obj 中 mpen 惩罚 1e4×s 主导、tardy_w 权重过低）为让中继顺排把运输架次整体推迟，运输完工 7666→9351~9982s、迟到 641~4118s。
- **修复**：零迟到硬罚（tardy_w>0 → obj+=1e7）优先于中继惩罚。修复后 28 架 seed77 找到 **零迟到 + 零覆盖 + 零 R2 重叠** 解（mpen 171s 为残留中继时序余量差）。
- **根因**：固定 W/E/N 位置下中继时间线天然紧，SA 宁牺牲运输准时换取 mpen=0。

### D2【结构结论·非 bug】25 架能耗冠军零迟到联合不可达
- 25 架（7790.5/78.88）零迟到硬罚下 SA 两 seed 均只得迟到 144.7~660s 的解（联合 8090.5s）——**中继固定 3 位置下无法零迟到完成 P3 联合**；28 架完工冠军则可行（9369.0s 零迟到）。
- 论文口径建议：能耗优先方案需声明 P3 联合期望迟到 ~145s（软指标），或直接采用 28 架完工冠军做联合主结果。

### D3【次要】dispatch_compliant 极端 releases 偶发 None
- 极端偏移组合（某型号全部电池未充满）下无可用电池时序返回 None——`p3v28_compliant.py` 已安全降级（evaluate 返 1e9、finalize 回退全 0 偏移）。根因属"该组合下真不可行"，非逻辑错误。

### D4【合规 P3 联合最终数字】（PYTHONHASHSEED=0、SA 3500 iters、零迟到硬罚）
| 方案 | seed | 联合完工 | 运输完工 | 迟到 | mpen | 电池违规 |
|---|---|---|---|---|---|---|
| 28架完工优先 | 77 | **9369.0 s (156.2 min)** | 9351.4 | **0** | 171 s | 0 |
| 25架能耗优先 | 7/77 | 8090.5 s (134.8 min) | 8090.5 | 144.7~660 | 0 | 0 |

相对旧宽松口径：28架 9304.9 → **9369.0（+64.1s）**；25架 7939 → 8090.5（+151.5s）。

---

## E. B2 中继悬停位置自由化实验（v29，`实验/p3v29_relayfree_probe.py` / `p3v29_relayfree.py`）

### E1 侦察结论（28 架合规冠军需中继采样点，`p3v29_probe_makespan.json`）
- 需中继采样点按区：S001=24, S002=188, S003=140, S004=125, S005=288, S007=206, S008=115, S009=176, S010=83, S012=56, S013=105, S014=114, S015=142（S006/S011 直连无需中继）。
- **原 W/E/N 三位置已对各自组 100% 全覆盖**（W 1279/1279、E 358/358、N 125/125）→ cover_bad=0 不是 mpen 根源。
- 组级网格搜索（半径 ±0.025°、步长 0.004° + 精化，离地 100–300m 逐层）新位置亦全覆盖，但**转场往返时间与原位置几乎相同**：
  | 组 | 原位置往返 | 新位置往返 | 差额 |
  |---|---|---|---|
  | W | 812 s | 840 s | +28 |
  | E | 695 s | 689 s | -6 |
  | N | 890 s | 917 s | +27 |
- **分组合并不可行**（任何 2 位置组合均无法同时全覆盖三组）：E 位置对 N 覆盖 0/125、N 位置对 E 覆盖 170/358、W 位置对 E 23/358、E 位置对 W 166/1279；两两组合（E+W / E+N / N+W）均缺一组 → **3 位置是地形/链路的必要条件**，E/N 无法用一架并点。
- 更大范围搜索（±0.05°）因 `relay_link_ok` 逐点 LOS 判定过慢（>600s/组）终止；结合上述局部证据，收益预期边际。

### E2 结论
**位置自由化收益边际**：① 现有 W/E/N 已是 v17 优化点（100% 覆盖 + 回程可行 + 低能耗）；② 网格搜索未找到显著更优位置（转场时间变化 ≤30s）；③ 3 组无法合并为 2 组。**mpen 171s 根因是 2 架中继的时间线串行衔接（E→N→W-tail 转场 + 组件满充复用），而非悬停坐标** —— 位置自由度无法消解，消解需靠中继任务再分配/更多中继架次（超出 B2 范围，受限于中继无人机 2 架与组件 6 的题给资源）。

### E3 SA 确认对比（28 架合规冠军，`experiment/p3v29_relayfree.py`，SA iters=1000、零迟到硬罚、seed 7/77、PYTHONHASHSEED=0）
| 位置 | seed | 联合完工 | 运输完工 | 迟到 | mpen | 中继能耗 | 电池违规 |
|---|---|---|---|---|---|---|---|
| **原 W/E/N**（对照组） | 77 | **9191.0 s** | 9173.3 | 0 | 802 | 4.676 | 0 |
| 新位置（搜索） | 77 | 9385.0 s | 9351.4 | 0 | 394 | 4.654 | 0 |
| 原 W/E/N（v28, 3500 iters） | 77 | 9369.0 s | 9351.4 | 0 | 171 | 4.585 | 0 |

- seed7 在 iters=1000 下未收敛（迟到 4847s），仅列 seed77（零迟到主 seed）。
- **读数**：位置更新（新位置）后 mpen 从 802→394（略降）但联合完工 9191→9385（略升），且两值均在 v28 的 mpen=171/9369.0 上下波动——**位置自由度对联合完工的影响 ≈0，处于 SA 随机级噪声**；mpen 无法归零，证实 §E2（时间线张力非位置几何）。
- **修法（论文口径）**：头号结果继续采用 v28 固定 W/E/N 的 **9369.0 s（156.2 min）/ 零迟到 / mpen 171 s**；位置自由化实验作为"敏感性/鲁棒性分析"写入论文（结论：悬停位置在 DEM 约束下的最优性对当前 3 点已近似饱和，中继协同瓶颈在架次时间线而非位置）。
